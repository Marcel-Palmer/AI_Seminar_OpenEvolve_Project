"""
Evaluator for Coxeter Radical example with improved timeout handling
"""

import importlib.util
import numpy as np

# Our individual imports
########################
import networkx as nx
import sympy as sp
import matplotlib as plt
import scipy
import copy
########################

import time
import os
import signal
import subprocess
import tempfile
import traceback
import sys
import pickle


class TimeoutError(Exception):
    pass


def timeout_handler(signum, frame):
    """Handle timeout signal"""
    raise TimeoutError("Function execution timed out")


def validate_graph(G,k=2):                                                                                #------> changed
    """
    Validate that our graph yieds a radical greater equal to k

    Args:
        graph: nx.Graph object
        k: integer (default: 2)

    Returns:
        True if valid, False otherwise
    """
    # Our validation code here !!!
    valid = True
    if G.number_of_nodes()+G.number_of_edges() > 0 and G.number_of_nodes()+G.number_of_edges() <= 50:

        if nx.is_connected(G):
            adj_mat = nx.adjacency_matrix(G).toarray()
            n = len(adj_mat)

            # Adj-matrix to Cox-matrix
            Cox_mat = copy.copy(adj_mat)
            
            for i in range(0,n):
                for j in range(0,n):
                    if i != j and Cox_mat[i,j] == 0:
                        Cox_mat[i,j]= 2

                    elif Cox_mat[i,j] == 1:
                        Cox_mat[i,j] = 3

                    elif i == j:
                        Cox_mat[i,j] = 1

            # Cox-matrix to Cos-matrix
            Cos_mat = sp.zeros(n,n)
            for i in range(n):
                for j in range(n):
                    Cos_mat[i,j] = (-sp.cos(sp.pi/Cox_mat[i][j]))

            if Cos_mat.det() == 0:
                r = Cos_mat.rank()
                if n-r < k:
                    valid = False
            else:
                valid = False
        else:
            valid = False
    else:
        valid = False

    return valid


def run_with_timeout(program_path, timeout_seconds=20):
    """
    Run the program in a separate process with timeout
    using a simple subprocess approach

    Args:
        program_path: Path to the program file
        timeout_seconds: Maximum execution time in seconds

    Returns:
        graph, num vertices, num edges, sum tuple from the program
    """
    # Create a temporary file to execute
    with tempfile.NamedTemporaryFile(suffix=".py", delete=False) as temp_file:
        # Write a script that executes the program and saves results
        script = f"""
import sys
import numpy as np
import networkx as nx                                                                                         #------> added
import matplotlib as plt                                                                                     #------> added
import copy                                                                                                 #------> added
import os
import pickle
import traceback

# Add the directory to sys.path
sys.path.insert(0, os.path.dirname('{program_path}'))

# Debugging info
print(f"Running in subprocess, Python version: {{sys.version}}")
print(f"Program path: {program_path}")

try:
    # Import the program
    spec = __import__('importlib.util').util.spec_from_file_location("program", '{program_path}')
    program = __import__('importlib.util').util.module_from_spec(spec)
    spec.loader.exec_module(program)

    # Run the graph function
    print("Calling run_graph()...")                                                                       #------> changed
    G, num_v, num_e, sum = program.run_graph()                                                            #------> changed
    print(f"run_graph() returned successfully: sum = {{sum}}")                                            #------> changed

    # Save results to a file
    results = {{
        'Graph': G,
        'Num_Vertices': num_v,
        'Num_Edges': num_e,
        'Sum': sum
    }}                                                                                                    #------> changed

    with open('{temp_file.name}.results', 'wb') as f:
        pickle.dump(results, f)
    print(f"Results saved to {temp_file.name}.results")

except Exception as e:
    # If an error occurs, save the error instead
    print(f"Error in subprocess: {{str(e)}}")
    traceback.print_exc()
    with open('{temp_file.name}.results', 'wb') as f:
        pickle.dump({{'error': str(e)}}, f)
    print(f"Error saved to {temp_file.name}.results")
"""
        temp_file.write(script.encode())
        temp_file_path = temp_file.name

    results_path = f"{temp_file_path}.results"

    try:
        # Run the script with timeout
        process = subprocess.Popen(
            [sys.executable, temp_file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )

        try:
            stdout, stderr = process.communicate(timeout=timeout_seconds)
            exit_code = process.returncode

            # Always print output for debugging purposes
            print(f"Subprocess stdout: {stdout.decode()}")
            if stderr:
                print(f"Subprocess stderr: {stderr.decode()}")

            # Still raise an error for non-zero exit codes, but only after printing the output
            if exit_code != 0:
                raise RuntimeError(f"Process exited with code {exit_code}")

            # Load the results
            if os.path.exists(results_path):
                with open(results_path, "rb") as f:
                    results = pickle.load(f)

                # Check if an error was returned
                if "error" in results:
                    raise RuntimeError(f"Program execution failed: {results['error']}")

                return results["Graph"], results["Num_Vertices"], results["Num_Edges"], results["Sum"]  #------> changed
            else:
                raise RuntimeError("Results file not found")

        except subprocess.TimeoutExpired:
            # Kill the process if it times out
            process.kill()
            process.wait()
            raise TimeoutError(f"Process timed out after {timeout_seconds} seconds")

    finally:
        # Clean up temporary files
        if os.path.exists(temp_file_path):
            os.unlink(temp_file_path)
        if os.path.exists(results_path):
            os.unlink(results_path)


def evaluate(program_path):
    """
    Evaluate the program by running it once and checking the sum of num vertices
    and num edges

    Args:
        program_path: Path to the program file

    Returns:
        Dictionary of metrics
    """
    # Target value from the paper
    INIT_VALUE = 28 # Init sum = sum_v+num_e of Init Graph                                                 #------> changed

    try:
        # For constructor-based approaches, a single evaluation is sufficient
        # since the result is deterministic
        start_time = time.time()

        # Use subprocess to run with timeout
        G, num_v, num_e, reported_sum = run_with_timeout(
            program_path, timeout_seconds=600  # Single timeout
        )                                                                                                     #------> changed

        end_time = time.time()
        eval_time = end_time - start_time

        # Ensure centers and radii are numpy arrays
        # if not isinstance(centers, np.ndarray):
        #     centers = np.array(centers)
        # if not isinstance(radii, np.ndarray):
        #     radii = np.array(radii)                                                                         #------> changed (not needed)

        # Check for NaN values before validation
        if np.isnan(num_v).any() or np.isnan(num_e).any():                                                    #------> changed (check np.isnan)
            print("NaN values detected in solution")
            return {
                "sum": 1000,
                "init_diff": 1000,
                "validity": 0,
                "eval_time": float(time.time() - start_time),
                "combined_score": 1000,
            }                                                                                                 #------> changed (maybe different values)

        # Validate solution
        valid = validate_graph(G)

        # Check shape and size
        # shape_valid = centers.shape == (26, 2) and radii.shape == (26,)
        # if not shape_valid:
        #     print(
        #         f"Invalid shapes: centers={centers.shape}, radii={radii.shape}, expected (26, 2) and (26,)"
        #     )
        #     valid = False                                                                                   #------> changed (not needed)

        # Calculate sum
        sum = num_v + num_e if valid else 1000                                                                #------> changed

        # Make sure reported_sum matches the calculated sum
        if abs(sum - reported_sum) > 0:                                                                       #------> changed
            print(f"Warning: Reported sum {reported_sum} doesn't match calculated sum {sum}")                 #------> changed

        # Target ratio (how close we are to the target)
        init_diff = INIT_VALUE-sum if valid else 1000                                                         #------> changed

        # Validity score
        validity = 1 if valid else 0                                                                       #------> changed

        # Combined score - higher is better
        combined_score = init_diff * validity                                                                 #------> changed

        print(
            f"Evaluation: valid={valid}, sum={sum}, init={INIT_VALUE}, diff={init_diff}, time={eval_time:.2f}s"   #------> changed
        )

        return {
            "sum": sum,
            "init_diff": init_diff,
            "validity": validity,
            "eval_time": float(eval_time),
            "combined_score": combined_score,
        }                                                                                                     #------> changed

    except Exception as e:
        print(f"Evaluation failed completely: {str(e)}")
        traceback.print_exc()
        return {
            "sum": 1000,
            "init_diff": 1000,
            "validity": 0,
            "eval_time": 0.0,
            "combined_score": 1000,
        }                                                                                                     #------> changed (maybe different values)
    
